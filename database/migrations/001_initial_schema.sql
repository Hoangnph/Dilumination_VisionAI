...
-- Description: Create initial database schema for people counter system
-- Version: 1.0
-- Created: 2025-09-04
...
CREATE INDEX IF NOT EXISTS idx_people_movements_session_id ON people_movements(session_id);
CREATE INDEX IF NOT EXISTS idx_people_movements_time ON people_movements(movement_time);
CREATE INDEX IF NOT EXISTS idx_people_movements_direction ON people_movements(movement_direction);
CREATE INDEX IF NOT EXISTS idx_people_movements_person_id ON people_movements(person_id);
...
CREATE INDEX IF NOT EXISTS idx_realtime_session_id ON realtime_metrics(session_id);
CREATE INDEX IF NOT EXISTS idx_realtime_timestamp ON realtime_metrics(timestamp);
CREATE INDEX IF NOT EXISTS idx_realtime_session_timestamp ON realtime_metrics(session_id, timestamp);

CREATE INDEX IF NOT EXISTS idx_hourly_session_hour ON hourly_statistics(session_id, hour_timestamp);
CREATE INDEX IF NOT EXISTS idx_daily_session_date ON daily_statistics(session_id, date_timestamp);
...
CREATE INDEX IF NOT EXISTS idx_alert_logs_resolved ON alert_logs(is_resolved);
...
DROP TRIGGER IF EXISTS update_session_statistics_updated_at ON session_statistics;
...
-- Create function to calculate current people inside
...
-- Create trigger to update statistics when movement is recorded
...
CREATE OR REPLACE VIEW current_session_overview AS
SELECT 
    s.id,
    s.session_name,
    s.status,
...
VALUES ('migration_001_completed', 'true', 'boolean', 'Initial schema migration completed')
